...
const SelectCombo = ({ products, onVariationSelect, selectedVariations = {}, selectedAttributes: externalSelectedAttributes = [], onRemoveProduct }) => {
    const [selectedAttributes, setSelectedAttributes] = useState(() =>
        (externalSelectedAttributes && externalSelectedAttributes.length > 0) ? externalSelectedAttributes : (products || []).map(() => ({}))
    );
    ...
    useEffect(() => {
        const newAttributes = (externalSelectedAttributes && externalSelectedAttributes.length > 0) ? externalSelectedAttributes : (products || []).map(() => ({}));
    ...
    return (
        <div className='w-full flex flex-col gap-2 rounded'>
            {(products || []).map((p, index) => {
                if (!p) return null;
                const images = parseJSON(p.featuredImage);
