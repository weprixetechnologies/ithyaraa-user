...
const SignUpAction = () => {
    ...
    const sendOtp = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://eighty-taxes-dress.loca.lt/api'}/user/send-otp`, {
                method: 'POST',
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            // verify OTP
            const verifyRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://eighty-taxes-dress.loca.lt/api'}/user/verify-otp`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            // create user
            const createRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://eighty-taxes-dress.loca.lt/api'}/user/create-user`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    password,
