                ...
                <div className="pdp-modal-body">
                    <div className="pdp-picker-grid">
                        {availableProducts.map((product) => {
                            ...
                            const attrs = selectedAttributes[product.productID] || {};
                            const filtered = getFilteredVariations(product.variations, product.productID);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="pdp-picker-info">
                                        <p className="pdp-picker-name">{product.name}</p>
                                        <div className="pdp-picker-stars">
                                            {[1, 2, 3, 4, 5].map(s => (
                                                <BsFillStarFill key={s} className={s <= (product.rating || 4) ? "pdp-star-on" : "pdp-star-off"} />
                                            ))}
                                            <span className="pdp-picker-rating">{product.rating || 4.5}</span>
                                        </div>
                                        <p className="pdp-picker-price">₹{product.salePrice || product.regularPrice}</p>
                                                        ...
                                                        <div className="pdp-picker-attr-vals">
                                                            {attr.values.map((val, vi) => (
                                                                <button
                                                                    ...
                                                                    onClick={(e) => { e.stopPropagation(); onAttributeSelect(product.productID, attr.name, val); }}
                                                                >
                                                                    {val}
                                                                </button>
                                                            ))}
...
const ProductDetail = () => {
    ...
    useEffect(() => {
        if (!productID) return;
        axiosInstance.get(`/reviews/product/${productID}/stats`)
        ...
        (async () => {
            try {
                ...
                for (const id of shuffled) {
                    ...
                    seen.add(id);
                    try {
                        const products = await fetchSectionProducts({ sectionid: id });
                        if (products.length > 0) results.push({ id, products, ...SECTION_META[id] });
                    } catch { }
                }
                setSections(results);
            } catch { }
    ...
    useEffect(() => {
        selectedProducts.forEach(product => {
            if (!selectedAttributes[product.productID]) return;
            const match = (product.variations || []).find(v =>
                (Array.isArray(v.variationValues) ? v.variationValues : []).every(obj => {
                    const [k, val] = Object.entries(obj)[0];
                    return selectedAttributes[product.productID]?.[k] === val;
                })
            );
            if (match && match.variationStock > 0 && selectedVariations[product.productID] !== match.variationID) {
                setSelectedVariations(prev => ({ ...prev, [product.productID]: match.variationID }));
            }
        });
    ...
    const toggleProductSelection = (product) => {
        const already = selectedProducts.find(p => p.productID === product.productID);
        if (already) {
            setSelectedProducts(prev => prev.filter(p => p.productID !== product.productID));
            setSelectedVariations(prev => { const n = { ...prev }; delete n[product.productID]; return n; });
            setSelectedAttributes(prev => { const n = { ...prev }; delete n[product.productID]; return n; });
        } else {
    ...
    if (!comboData) return <p style={{ textAlign: "center", marginTop: 40 }}>Combo not found</p>;

    return (
        <>
            {/* ── Global styles — identical token system to variable PDP ── */}
            <style>{`
                :root {
                    --pdp-cream:    #faf8f4;
                    --pdp-warm:     #f4f0e8;
                ...
                .pdp-selection-count {
                    font-size: 11px; color: var(--pdp-muted);
                }
                ...
                .pdp-add-more:hover {
                    ...
                    background: linear-gradient(135deg, rgba(184,148,58,.09), rgba(184,148,58,.15));
                    color: var(--pdp-ink);
                }
                ...
                .pdp-btn-cart {
                    ...
                    cursor: pointer; transition: border-color .2s, box-shadow .2s; padding: 0 12px;
                ...
                .pdp-btn-cart .pdp-ripple {
                    position:absolute; border-radius:50%; background:rgba(28,24,18,.15);
                ...
                .pdp-modal-overlay {
                    ...
                    animation: pdpOverlayIn .2s ease both;
                }
                ...
                .pdp-modal {
                    ...
                    box-shadow: 0 32px 80px rgba(14,12,10,.28), 0 8px 24px rgba(14,12,10,.12);
                    animation: pdpModalIn .35s cubic-bezier(.22,1,.36,1) both;
                    overflow: hidden;
                }
                ...
                .pdp-modal-eyebrow {
                    ...
                    text-transform: uppercase; color: var(--pdp-gold); margin: 0 0 4px;
                }
                .pdp-modal-title {
                    font-size: 22px; font-weight: 400; color: var(--pdp-ink);
                    letter-spacing: -.02em; margin: 0;
                }
                .pdp-modal-close {
                    width: 34px; height: 34px; border-radius: 50%;
                    border: 1px solid var(--pdp-border); background: #fff;
                ...
                .pdp-picker-card {
                    ...
                    border: 1.5px solid var(--pdp-border); border-radius: 14px;
                ...
                .pdp-picker-img-wrap {
                    ...
                    border-radius: 10px; overflow: hidden; border: 1px solid var(--pdp-border);
                    background: var(--pdp-warm);
                }
                .pdp-picker-img   { width: 100%; height: 100%; object-fit: cover; }
                .pdp-picker-check {
                    position: absolute; inset: 0; background: rgba(184,148,58,.85);
                    display: flex; align-items: center; justify-content: center;
                    color: #fff; font-size: 20px; font-weight: 700;
                    animation: pdpFadeUp .2s ease both;
                }
                ...
                .pdp-picker-attr-btn {
                    ...
                    color: var(--pdp-ink); cursor: pointer; transition: all .15s;
                }
                ...
                .pdp-btn-outline {
                    padding: 10px 18px; border: 1.5px solid var(--pdp-border); border-radius: 10px;
                    font-size: 13px; font-weight: 500; cursor: pointer; background: #fff;
                    color: var(--pdp-ink); transition: all .15s;
                }
                ...
                .pdp-btn-confirm {
                    ...
                    background: var(--pdp-ink); color: #fff;
            ...
            <div className="pdp-root">
                <div style={{ maxWidth: 1160, margin: "0 auto", padding: "32px 20px 72px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 40, alignItems: "start" }}>
                        ...
                        <div>
                            ...
                            <div className="pdp-fade-up pdp-fade-up-d1">
                                {/* <span className="pdp-brand-pill">{comboData.brand || "Collection"}</span> */}
                    ...
                    {/* ── Sections + Reviews (identical pattern to variable PDP) ── */}
                    {isMounted && (
                        <>
                            {sectionAbove && (
                                <>
